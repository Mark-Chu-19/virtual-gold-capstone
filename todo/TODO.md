...
| 9 | **Use case / sector** | Client said on Sep 3 that a sector focus (e.g. financial or real estate) is welcome and that data will be provided once the use case is set. Proposed default: financial-services SMB document workflows (Q&A, summarization, extraction). Confirm with the client. | | W4 | [ ] |
...
| 11 | Set up the sandbox: Ollama plus the two models on the GPU host; laptops get only the client code and a small dev model | | W4 | [ ] |
...
| 26 | Section 6: add cost ownership once B14 is answered, e.g. "The Client will provide cloud model API credits up to $X and sandbox compute; otherwise compute is provided through CMU Public Cloud Services." | [ ] |
