...
|---|---|---|---|---|---|
| 1 | **Tool selection** (partly decided) | **Decided 2026-09-15 (Mark): local inference and the evaluation harness run on HF transformers** (8B at 4-bit via bitsandbytes on the T4), because the harness needs per-token logprobs and hidden states for the Semantic Entropy Probe (PR #2); Ollama, llama.cpp server and vLLM do not expose hidden states. Ollama is not used. **Presidio** for PII detection: keep; independent of the inference stack. **LiteLLM**: proposed default is to drop it for the MVP. It is an HTTP adapter over OpenAI-style APIs; with transformers in-process the local side has no API to adapt, and its hooks route pre-call between deployments, not post-generation on confidence. The router is a Python module that calls transformers locally and the cloud provider's SDK (A6). **RouteLLM**: no conflict, but a different signal (pre-generation, looks at the query) from our post-generation confidence scoring; keep as the stretch-goal comparison (task-aware routing, architecture §9), not in the MVP. Confirm the LiteLLM and RouteLLM defaults at the team meeting. | Mark | W4 | [ ] |
