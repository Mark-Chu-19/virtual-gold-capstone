...
| 7 | **What to show at the W7 midpoint** | **Set 2026-09-15.** (1) Three configurations, local-only / hybrid / cloud-only, on the MMLU subset and GSM8K: accuracy, escalation rate, cost per query, latency (PII-leak rate and prompt-injection resistance come in W9–W11). Cloud-only falls out of the same run because every query is sent to the cloud once and cached (PR #2). (2) Confidence signal: token logprob only (PR #2 variant A); SEP and the sampling ensemble in W9–W11. (3) One chart: escalation rate vs accuracy, the direct answer to "how much escalation is enough". (4) A CLI demo of one request containing PII going through redaction, escalation and re-identification (A3, A4). Deck ready in W6. | Mark | W6 | [ ] |
| 8 | **Use case / sector** | **To settle with the client at the 2026-09-15 meeting.** On Sep 3 the client said a sector focus (e.g. financial or real estate) is welcome and that data follows once the use case is set. Team default to propose: financial-services SMB document workflows (Q&A, summarization, extraction). Ask: (a) does financial services work, or do they prefer real estate or another sector; (b) what example documents or formats they can share for that sector (D22). Note for the meeting: the choice does not affect the benchmark numbers (MMLU, GSM8K are sector-neutral); it sets the background for the synthetic data (B14) and the PII demo (A7), and RAG is a stretch goal (A2), so we are not blocked on their data. | Mark | 2026-09-15 | [ ] |
| 9 | **Work split and research** | **Owners to be claimed at the 2026-09-15 team meeting.** Five workstreams, one owner each, scope updated for A1–A8: (1) **Router and integration**: our own Python router (confidence threshold plus the confidential-stays-local rule, A3), cloud SDK call (A6), CLI entry point. (2) **Models and inference**: HF transformers + bitsandbytes 4-bit for Llama 3.1 8B and Qwen3 8B, sandbox setup (B12), throughput and memory on the sandbox GPU. (3) **Confidence scoring and evaluation harness**: owner Zhexuan Ye (research report and design doc delivered, PR #2); sampling core, benchmark loaders, metrics, midpoint pilot. (4) **Security and de-identification**: Presidio gate, re-identification mapping table (A4), PII-leak and prompt-injection evaluation in W9–W11. (5) **Synthetic data (RAG deferred to stretch, A2)**: synthetic enterprise queries and documents for the demo and B14, F32 one-pager; lightest load before W7, so this owner also supports (3) or the midpoint deck. Each workstream starts with the F-section research one-pager. Docs and slides: each owner writes their part, Mark consolidates. | Mark | 2026-09-15 | [ ] |
...
| 11 | **Bump the architecture doc to v0.3.** Conditions (A1–A4 decided, client answers on B10) are met on 2026-09-15. Start after PR #2 merges to avoid editing the same sections twice. Changes: §5 add the verbalized-confidence exclusion and the signal priority (token logprob → SEP → sampling ensemble); §7 add the confidence-metrics sub-table and the cloud-once-per-query cache; §9 RAG moves to stretch, sensitivity rule and re-identification mapping table listed in the MVP; §10 fold in the client's answers; §11 tick A1–A4, remove LiteLLM / RouteLLM / Ollama / CMU fallback; version table adds v0.3. All four formats (EN/zh md and html). | Mark | W4 | [ ] |
