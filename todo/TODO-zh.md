...
| 9 | **分工** | 路由 / 模型與推論 / 評測 harness / 安全與去識別化 / 文件與簡報,五人各認領一塊。 | | | [ ] |
...
| 16 | **詢問團隊能否申請 CMU GitHub Enterprise Cloud 的 organization。**免費且附 branch protection,但該服務定位為校內專案,且需教職員代為申請。跟第 15 項寫在同一封信裡問。可以的話把倉庫搬過去並鎖住 main;不行就維持現在約定制的 PR 流程。 | Mark | W4 | [ ] |
...
| # | 修正 | 完成 |
|---|---|---|
| 17 | 「Llama 3.3 8B」改為 Llama 3.1 8B(Llama 3.3 只有 70B) | [ ] |
| 18 | 「Qwen3 7B」改為 Qwen3 8B(7B 是 Qwen2.5) | [ ] |
| 19 | 圖:「高信心直接回覆」的箭頭不應經過 PII 遮罩閘道 | [ ] |
| 20 | 模型數據改引 Meta 與 Qwen 官方 model card,框架改引 NIST AI RMF 原文,不引部落格 | [ ] |
| 21 | 時程對齊課程:W7 期中、W8 秋假、W14 感恩節 | [ ] |
...
|---|---|---|---|
| 22 | 確認或修改第十節的八項預設方案 | | |
| 23 | 提案中提到的「企業與小企業資料」有沒有範例文件或格式 | | |
| 24 | 國際開源模型(Qwen3)可否納入評估 | | |
...
| # | 修改 | 完成 |
|---|---|---|
| 25 | 第 6 節假設第 3 條:「開發與實驗主要在學生筆電、桌機或其他核准的沙盒環境進行」改為「開發與實驗主要在客戶或 CMU Computing Services 提供的雲端沙盒環境進行,該環境設定為模擬企業地端部署;學生筆電只用於開發」。 | [ ] |
| 26 | 第 6 節新增依賴:「客戶將提供雲端沙盒環境供測試使用,如 2026 年 9 月 3 日會議所討論」。 | [ ] |
| 27 | 第 6 節在 B15 有答案後補上費用歸屬,例如「客戶提供雲端模型 API 額度上限 X 美元與沙盒算力;若無,算力由 CMU Public Cloud Services 提供」。 | [ ] |
| 28 | 第 5 節 Phase 1 末尾加上「並開始早期原型驗證架構假設」(Alex 於 9 月 3 日提出)。 | [ ] |
