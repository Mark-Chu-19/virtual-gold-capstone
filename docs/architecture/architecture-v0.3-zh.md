...
**Capstone · Virtual Gold Inc · 第 4 週 · Project Plan**

本地優先的混合式企業 AI 助理:以開源模型為主要智慧層,依信心分數與資料敏感度,選擇性升級到雲端模型。

| 版本 | 日期 | 狀態 | 團隊 | 客戶端 |
|---|---|---|---|---|
| v0.3 | 2026-09-15 | 已記錄 9 月 15 日團隊決策;待客戶回覆 | Anmol · Mark · Yudi · Zhexuan · Karina | Inderpal Bhandari · Alex Bhandari · Urte Jesina |
...
flowchart TB
  subgraph LOCAL["本地環境 · 企業內部網路"]
    ...
    L6["6 知識層(RAG,stretch goal)<br/>本地 Embedding · Chroma / Qdrant / pgvector"]
...
| 6 | 知識層(RAG),stretch goal | 本地 Embedding;Chroma / Qdrant / pgvector。不在 MVP(2026-09-15 決定) |
...
2. 路由器對任務分類(問答、摘要、程式、推理等);從 RAG 撈文件片段列為 stretch goal。
...
> **期中目標(2026-09-15 定案):**W7 期中簡報前,三配置都在 MMLU 子集與 GSM8K 上跑完,信心訊號只用 token logprob,報告準確率、升級率、每題成本與延遲。每題送雲端一次並快取,cloud-only 從同一次跑產出。其餘信心策略、PII 洩漏率與 prompt injection 抵抗力放在第 9 到 11 週。
...
| 週次 | 階段 | 範圍 |
|---|---|---|
| W1–W3 | 準備 | 組隊、客戶 Kickoff、架構定案 |
| W4–W7 | **MVP(本週起)** | 本地模型 + 基本信心策略 + 路由 + PII 閘道 + 評測 harness。**W7 期中簡報。** |
| W8 | 秋假 | — |
| W9–W11 | **擴充** | 三配置完整評測、其餘信心策略、紅隊測試、保留格式的假值 |
| W12–W15 | **收斂** | Benchmark 報告、治理建議、草稿交付、客戶回饋、**W15 最終簡報**(W14 感恩節) |

客戶會議週一 18:00–19:00(初期每週,專案定義清楚後改為雙週);團隊會議週五 15:30–16:30;與教授的會議週二 17:00–18:20。由 Mark 排程。

### MVP:團隊承諾交付

五個人都是全職研究生並修滿課程,這個專案約占一學期三分之一的學分。因此目標是「可運作的原型加嚴謹的評測」,不是產品級平台。盡量用現成的開源元件,把力氣放在整合、評測與安全治理分析。

- **路由與編排:**自寫 Python router:信心門檻加「機密留本地」規則,本地在程式內呼叫模型,雲端呼叫供應商 SDK。不用閘道框架(2026-09-15 決定)。
- **本地推論:**Llama 3.1 8B 為主力,Qwen3 8B 為程式與多語言題目的備選;以 HF transformers(bitsandbytes 4-bit)載入,跑在模擬地端的 GPU 沙盒上(見下方「部署環境」)。團隊 2026-09-10 決定:不在筆電上跑 LLM,筆電只做開發。
- **信心評分:**期中以 token logprob 為基準線;第五節其餘策略在第 9 到 11 週比較。
- **PII 遮罩閘道與還原:**Presidio 遮罩位於路由與任何雲端呼叫之間,並以每次請求的佔位符對照表還原回覆(2026-09-15 決定);數值類用佔位符。
- **雲端升級:**一家主流企業級 API,只在低信心時呼叫。
- **評測與日誌 harness:**第七節的三配置與六指標。
- **介面:**不做圖形介面,只做 CLI 或 REST API(9 月 3 日決議,客戶要求優先架構本身)。
- **資料敏感度分級:**三級;機密不論信心高低都不外送(2026-09-15 決定)。
- **RAG 知識層:**stretch goal,不在 MVP(2026-09-15 決定)。

### Stretch goals:MVP 提早完成才做

- 任務感知路由:依問題類型與複雜度路由,而非只看信心分數,以 RouterBench 方法評估。
- 在合成企業文件上建最簡版 RAG 知識層,並以檢索接地度作為額外信心訊號(2026-09-15 延後)。
...
團隊於 2026-09-10 決定不在筆電上跑 LLM。模型改跑在雲端沙盒的 GPU 主機上,客戶在 9 月 3 日會議承諾提供沙盒;筆電只負責開發。地端的本質是「誰控制它、資料能不能出去」,所以沙盒要模擬的是那條邊界,而且要能被驗證。

```
internal 網路(無對外連線)          egress 網路(只到白名單)
├── llm-server  本地模型(HF transformers)└── egress-proxy  唯一出口,記錄每次請求
├── chroma      向量庫(RAG,stretch)         ↑
├── presidio    去識別化                     │
├── audit-db    稽核日誌                     │
└── router  ── 同時掛在兩個網路 ─────────────┘
```
...
| 雲端供應商 | MVP 只接一家:客戶提供哪家的 API 額度或企業帳號就用哪家;客戶沒有偏好時用 Anthropic。兩家都公開企業級資料保留承諾。 | 減少變數;多供應商比較列為 stretch。 | 指定供應商、地區或合規限制。 |
| 國際開源模型 | Qwen3 8B 作為備選並納入供應鏈檢查(來源、hash、授權)。 | 客戶提案明確關切國際模型風險,納入評估才能給結論。 | 可要求排除。 |
| 硬體與運算環境 | 模型跑在 GPU 沙盒(T4 等級,16GB)上模擬地端;8B 級模型以 HF transformers 載入,4-bit(bitsandbytes)。筆電不跑 LLM。 | 團隊 2026-09-10 決定;客戶 9 月 3 日承諾提供沙盒。 | 確認沙盒規格與費用;提供 A10G 級(24 GB)則 k 次採樣的信心集成可從容跑,並可測 14B。 |
...
| 費用負擔 | 雲端模型 API 額度與沙盒算力由客戶提供。 | SOW v2 第 6 節已寫明由客戶負擔;金額待確認。 | 指定額度上限或供應商帳號。 |

## 十一、團隊待討論事項(代辦)

追蹤於 `todo/TODO-zh.md`;本清單為 2026-09-15 的對照。

- [x] **工具選型。**2026-09-15 決定:本地推論與評測 harness 都用 HF transformers(bitsandbytes 4-bit),因為 harness 需要每個 token 的 logprobs 與 hidden states;router 自寫 Python 模組;Presidio 做 PII;雲端透過供應商 SDK 呼叫。不用 Ollama、LiteLLM、RouteLLM。
- [x] **RAG 知識層。**2026-09-15 決定:stretch goal,不在 MVP;W7 期中後再考慮。
- [x] **資料敏感度分級。**2026-09-15 決定:納入 MVP;機密請求不論信心高低都不外送。
- [x] **去識別化與還原管線。**2026-09-15 決定:兩段都進 MVP;還原為每次請求的佔位符對照表;數值類用佔位符,假值為 stretch。
- [ ] **本地模型與沙盒。**Llama 3.1 8B 主力、Qwen3 8B 備選;不在筆電跑 LLM。向客戶確認 9 月 3 日承諾的沙盒(GPU、直接存取、何時可用、費用)與雲端 API 額度。
- [ ] **雲端供應商。**客戶提供哪家額度就用哪家;沒有偏好時用 Anthropic。客戶回覆額度後定案。
- [x] **W7 期中要展示什麼。**2026-09-15 定案:第七節的期中目標,加一段 CLI demo:一個含 PII 的請求走完遮罩、升級、還原。
...
- [ ] **使用情境與產業。**預設金融服務類中小企業文件工作流;9 月 15 日客戶會議上確認。
- [ ] **分工。**五條工作流,一人一條:路由與整合、模型與推論、信心評分與評測 harness(Zhexuan)、安全與去識別化、合成資料。9 月 15 日團隊會議認領。

## 十二、下一步

1. 9 月 15 日客戶會議:當面走過第十節的預設方案;確認沙盒、API 額度、雲端供應商、使用情境與 SOW 費用金額。
2. 9 月 15 日團隊會議:認領工作流負責人;每條工作流第 4 週交研究一頁摘要。
3. 第 4 週開工:沙盒裝 HF transformers 並跑 smoke test,搭 harness 骨架(採樣核心、benchmark 載入、指標),備妥 MMLU 子集與 GSM8K 測試集;W7 前跑出三配置的第一批數據。

---

*Hybrid AI Assistant 架構草案 v0.3 · Capstone for Virtual Gold Inc · 2026-09-15*
